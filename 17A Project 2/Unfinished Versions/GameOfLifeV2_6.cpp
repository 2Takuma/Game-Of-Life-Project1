 ...
 * 
 *                      Main Focus:
 *                          finish up project code
 *                          get the main flow of the game going
 *                          figure out wtf is causing this error
 */
...
void sellH(Player &player) { // Sell House Function
    ...
    for (int j = 0; j < 5; j++) {
        if (strcmp(hcpy[j].getName(), "Null") != 0){
            int roll = spin();
...
void buyH(Player &player, House *arr, int size){
    ...
    do {
        ...
        if (pick == '1'){
            ...
            bool added = false;
            for (int i = 0; i < 5; i++){
                if (strcmp(cpyArr[i].getName(),"Null") == 0) {
                    cpyArr[i] = choice1;
                    added = true;
                    break;
                }
