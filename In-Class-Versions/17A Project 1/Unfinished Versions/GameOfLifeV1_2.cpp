 ...
 * Purpose:  Unfinished Version 1.2 
 *           Main changes: removed gameloop function as it would have too many parameters and cause confusion
 *           Added enum struct for stops and pay positions
 *           Worked on secondary file
 *           Fixed up code and added comments
 *           Commented ideas for functions other stuff
 *           Fix possible bugs
 */
...
void payChck(Player &, Career *, PayPos); // When user lands on pay position (or passes it)
...
int main(int argc, char** argv) {
    ...
    fread("Board.dat", board, 100); // Read in board file to array 
    
    cout << "Enter the amount of players you want playing (Max of 4 players):"; cin >> pCount;
    
    do {
        cout << "Please enter a valid amount of players (1-4): "; cin >> pCount;
    } while (!(pCount <= 4) || (pCount < 0)); // Check if player count is valid
    ...
    // Main game loop
    char input; // Input for player to press enter (for engagement)
    ...
    do {
        ...
        payChck(players[i]); // Call pay function (if they pass a pay position)
        ...
        else if (board[players[i].pos] == 'R'){ // Retirement position
            retire(players[i]); // Call retirement function
        }
        }
...
void payChck(Player &player, Career *career, PayPos places){ 
